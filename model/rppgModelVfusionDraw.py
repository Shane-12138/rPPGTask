...
class rPPGTR(nn.Module):
    ...
    def forward(self, video):
        ...
        frame = torch.sum(video[0, :, t], dim=0).detach().cpu().numpy()

        output_cnn = self.conv_patch(video)

        show1 = torch.sum(output_cnn[0, :, t], dim=0).detach().cpu().numpy()

        output_cnn = self.conv_block_1(output_cnn)

        show2 = torch.sum(output_cnn[0, :, t], dim=0).detach().cpu().numpy()

        output_cnn = self.conv_block_2(output_cnn)

        show3 = torch.sum(output_cnn[0, :, t], dim=0).detach().cpu().numpy()

        output_cnn = self.conv_block_3(output_cnn)

        show4 = torch.sum(output_cnn[0, :, t], dim=0).detach().cpu().numpy()

        output_bvp = self.mlp_bvp(output_cnn)
        output_hr = self.mlp_hr(output_cnn)

        show1 = cv2.resize(show1, (128, 128))
        show2 = cv2.resize(show2, (128, 128))
        show3 = cv2.resize(show3, (128, 128))
        show4 = cv2.resize(show4, (128, 128))
        # show = (show1 + show2 + show3 + show4) * frame
