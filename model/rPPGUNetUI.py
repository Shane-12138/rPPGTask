...
class Attention(nn.Module):
    def __init__(self, d_model, d_qkv):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=3, stride=1, padding=1),
...
class EncoderConv(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.encoder = nn.Sequential(
            # 普通卷积
            nn.Conv3d(in_channels=in_c, out_channels=out_c, kernel_size=3, stride=1, padding=1),
...
class Decoder(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.decoder = nn.Sequential(
            # 普通卷积
            nn.Conv1d(in_channels=in_c * 2, out_channels=out_c, kernel_size=3, stride=1, padding=1),
...
class UpSample4(nn.Module):
    def __init__(self, in_c):
        ...
        self.up_sample = nn.Sequential(
            nn.ConvTranspose1d(in_channels=in_c, out_channels=in_c, kernel_size=4, stride=4, padding=0),
            nn.BatchNorm1d(in_c),
            nn.ReLU()
        )
...
class UpSample2(nn.Module):
    def __init__(self, in_c):
        ...
        # 通道数不变，特征图面积翻倍
        self.up_sample = nn.Sequential(
            nn.ConvTranspose1d(in_channels=in_c, out_channels=in_c, kernel_size=2, stride=2, padding=0),
            nn.BatchNorm1d(in_c),
            nn.ReLU()
        )
...
class MlpHr(nn.Module):
    def __init__(self, in_c):
        ...
        self.mlp = nn.Sequential(
            nn.Conv1d(in_channels=in_c, out_channels=in_c, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(in_c),
            nn.ReLU(),
