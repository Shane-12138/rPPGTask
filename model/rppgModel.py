...
class FusionBlock(nn.Module):
    def __init__(self, d_model):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_channels=d_model * 2, out_channels=d_model, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(d_model),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x_res, x_attn):
        ...
        # y = self.conv(self.max(x_all) + self.avg(x_all) + x_all)
        y = self.conv(self.max(x_all) + self.avg(x_all))
        return y
